...
class AnonymizationIngesterPlugin(IIngesterPlugin):
    ...
    def __init__(self, anonymization_fields: Dict[str, List[str]], anonymizer: Anonymizer) -> None:
    ...
    def _anonymize_if_needed(self, destination_table, key: str, value: Any):
        ...
        if key and table in self._anonymization_fields and key in self._anonymization_fields[table]:
    ...
    def pre_ingest_process(
        ...
        if self._anonymization_fields:
            payload = [{k: self._anonymize_if_needed(destination_table, k, v) for (k, v) in item.items()} for item in
                       payload]
